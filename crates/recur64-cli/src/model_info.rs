...
/// Executed transformer blocks for the F10 R=1 control, used as the neural
/// compute baseline for the multiplier column.
const F10_R1_BASELINE_BLOCKS: f64 = 8.0;
...
pub fn run_model_info(path: &Path) -> anyhow::Result<()> {
    ...
    println!(
        "  {:<10} {:>16} {:>18} {:>12}",
        "R", "final blocks", "deep-sup blocks", "mult vs F10R1"
    );
    for r in &cfg.recurrence {
        ...
        let mult = final_blocks as f64 / F10_R1_BASELINE_BLOCKS;
        println!(
            "  R={:<8} {:>16} {:>18} {:>12.2}x",
    ...
    println!(
        "\nnote: 'mult vs F10R1' = executed_blocks / {:.0} (F10 R=1 control).",
        F10_R1_BASELINE_BLOCKS
    );
    Ok(())
}
